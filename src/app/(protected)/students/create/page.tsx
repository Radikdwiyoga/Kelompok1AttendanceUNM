...
export default function CreateStudentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClass) { setErrorMessage('Silakan pilih kelas.'); return; }
    setIsLoading(true);
    setErrorMessage('');
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMessage(result.message || 'Gagal menyimpan.');
      }
    } catch {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] leading-none mb-1.5">Manajemen Akademik</h3>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase leading-none">Tambah Mahasiswa</h1>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input required type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Masukkan Password (Default atau Buat Baru)..." className="w-full h-12 bg-slate-50 border-none rounded-xl px-5 text-sm font-bold text-slate-800 focus:ring-4 focus:ring-blue-500/10 transition-all font-mono tracking-widest" />
              </div>
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-50">
            ...
            <button type="submit" disabled={isLoading} className="flex-[2] h-14 bg-blue-600 text-white rounded-2xl flex items-center justify-center font-black text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-blue-500/10 hover:bg-blue-700 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <span>Simpan Data</span>}
            </button>
          </div>
        </form>
      </div>
